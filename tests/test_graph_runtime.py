...
def test_sanitize_attach_dsn_rejects_sql_control_tokens():
    with pytest.raises(ValueError, match="unsafe"):
        LadybugGraphRuntime._sanitize_attach_dsn("postgresql://u:p@localhost/db;DROP TABLE x")
    with pytest.raises(ValueError, match="unsafe"):
        LadybugGraphRuntime._sanitize_attach_dsn("postgresql://u:p@localhost/db -- comment")
