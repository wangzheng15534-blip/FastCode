...
kind_st = st.sampled_from(
    ["function", "method", "class", "variable", "module", "interface", "enum",
     "constant", "parameter", "type", "macro", "field"]
)
...
class TestBuildIrFromScipValidSnapshot:
    ...
    @pytest.mark.happy
    def test_produces_valid_ir_snapshot(self, index: SCIPIndex, repo: str, snap_id: str):
...
class TestEmptyAndEdgeCases:
    ...
    def test_raw_dict_input_produces_valid_snapshot(self, payload: dict, snap_id: str):
        ...
        snap = build_ir_from_scip(repo_name="repo", snapshot_id=snap_id, scip_index=payload)
        assert isinstance(snap, IRSnapshot)
        assert snap.repo_name == "repo"
        assert snap.snapshot_id == snap_id
    ...
    def test_document_missing_language_falls_back_to_hint(self, snap_id: str):
        ...
        index = SCIPIndex(documents=[
            SCIPDocument(path="f.txt", language=None, symbols=[
                SCIPSymbol(symbol="pkg foo.", name="foo"),
            ]),
        ])
        snap = build_ir_from_scip(
            repo_name="repo", snapshot_id=snap_id,
            scip_index=index, language_hint="rust",
        )
    ...
    @settings(max_examples=10)
    @pytest.mark.edge
    def test_document_missing_language_no_hint_falls_back_to_unknown(self, snap_id: str):
    ...
    def test_occurrence_empty_symbol_skipped(self, snap_id: str):
        ...
        index = SCIPIndex(documents=[
            SCIPDocument(path="a.py", language="python", occurrences=[
                SCIPOccurrence(symbol="", role="reference", range=[1, 0, 1, 10]),
                SCIPOccurrence(symbol="pkg foo.", role="definition", range=[2, 0, 2, 5]),
            ]),
        ])
        snap = build_ir_from_scip(repo_name="repo", snapshot_id=snap_id, scip_index=index)
...
class TestOccurrenceProperties:
    ...
    @settings(max_examples=20)
    @pytest.mark.happy
    def test_occurrence_range_none_coerced_to_zero(self, index: SCIPIndex, snap_id: str):
    ...
    def test_occurrence_ref_role_produces_ref_edge(self, snap_id: str):
        ...
        index = SCIPIndex(documents=[
            SCIPDocument(path="a.py", language="python", occurrences=[
                SCIPOccurrence(symbol="pkg foo.", role="reference", range=[1, 0, 1, 10]),
            ]),
        ])
        snap = build_ir_from_scip(repo_name="repo", snapshot_id=snap_id, scip_index=index)
