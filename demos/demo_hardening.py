...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="fc_hardening_") as tmp:
        ...
        print(
            f"  Validate token={token1}: {store.validate_fencing_token('index:snap:repo:1', token1)}"
        )
        print(
            f"  Validate stale token=999: {store.validate_fencing_token('index:snap:repo:1', 999)}"
        )
    ...
    worker.start()
    print(
        f"  Thread started: alive={worker._thread.is_alive()}, daemon={worker._thread.daemon}"
    )
    ...
    print(
        f"  Call graph: {graphs.call_graph.number_of_nodes()} nodes, {graphs.call_graph.number_of_edges()} edges"
    )

    # Callees from sym:auth
    dist = nx.single_source_shortest_path_length(
        graphs.call_graph, "sym:auth", cutoff=2
    )
    callees = [
