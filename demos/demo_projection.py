...
def _build_sample_snapshot() -> IRSnapshot:
    ...
    docs = [
        IRDocument(
            doc_id="doc:auth", path="app/auth.py", language="python", source_set={"ast"}
        ),
        IRDocument(
            doc_id="doc:user", path="app/user.py", language="python", source_set={"ast"}
        ),
        IRDocument(
            doc_id="doc:db", path="app/db.py", language="python", source_set={"ast"}
        ),
    ]
    symbols = [
        IRSymbol(
            symbol_id="sym:auth_svc",
        ...
        IRSymbol(
            symbol_id="sym:user_model",
            external_symbol_id=None,
            path="app/user.py",
            display_name="UserModel",
            kind="class",
            language="python",
            start_line=1,
            source_priority=10,
        ...
        IRSymbol(
            symbol_id="sym:db_conn",
            external_symbol_id=None,
            path="app/db.py",
            display_name="get_connection",
            kind="function",
            language="python",
            start_line=1,
            source_priority=10,
...
def main():
    ...
    print(
        f"Snapshot: {len(snapshot.documents)} docs, {len(snapshot.symbols)} symbols, {len(snapshot.edges)} edges"
    )

    graphs = IRGraphBuilder().build_graphs(snapshot)
    print(
        f"Graphs: dep={graphs.dependency_graph.number_of_edges()}, "
    ...
    scope = ProjectionScope(
        scope_kind="snapshot", snapshot_id="snap:demo:proj", scope_key="demo_key"
    )
    ...
    for chunk in result.chunks:
        print(
            f"  {chunk['chunk_id']}: {chunk['kind']} | version={chunk.get('version')} "
