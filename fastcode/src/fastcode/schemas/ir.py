"""
Canonical IR dataclasses: IRDocument, IRSymbol, IROccurrence, IREdge,
IRAttachment, and IRSnapshot.  Helper functions for resolution/confidence
...
class IRSnapshot:
    ...
    def documents(self) -> list[IRDocument]:
        ...
        for unit in self.units:
            if not isinstance(unit, IRCodeUnit):
                continue
            if unit.kind != "file":
                continue
            docs.append(
    ...
    def occurrences(self) -> list[IROccurrence]:
        ...
        file_units = {
            ...
            for unit in self.units
            if isinstance(unit, IRCodeUnit) and unit.kind == "file"
        }
        logical: dict[tuple[str, str, str, int, int, int, int], IROccurrence] = {}
        occurrences: list[IROccurrence] = []
        for support in self.supports:
            if not isinstance(support, IRUnitSupport):
                continue
            ...
            if not doc_id:
                unit = next(
                    (
                        u
                        for u in self.units
                        if isinstance(u, IRCodeUnit) and u.unit_id == support.unit_id
                    ),
                    None,
                )
                if unit:
    ...
    def edges(self) -> list[IREdge]:
        ...
        for relation in self.relations:
            if not isinstance(relation, IRRelation):
                continue
            edges.append(
                IREdge(
    ...
    def attachments(self) -> list[IRAttachment]:
        ...
        for embedding in self.embeddings:
            if not isinstance(embedding, IRUnitEmbedding):
                continue
            attachments.append(
                IRAttachment(
