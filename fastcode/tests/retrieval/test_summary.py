...
from fastcode.retrieval.summary import (
    extract_sources,
...
class TestGenerateFallbackSummary:
    ...
    def test_limits_files_list(self):
        elements = tuple(
            _mk_elem(f"f{i}", relative_path=f"file{i}.py") for i in range(20)
        )
...
class TestFormatAnswerWithSources:
    ...
    def test_no_sources(self):
        result = AnswerDisplayResult(answer="Simple answer")
        text = format_answer_with_sources(result)
