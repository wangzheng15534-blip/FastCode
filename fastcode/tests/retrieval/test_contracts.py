...
# --- FusionConfig typed contract ---


class TestFusionConfig:
    def test_defaults_are_typed_fields(self):
        cfg = FusionConfig()
        assert cfg.alpha_base == pytest.approx(0.8)
        assert cfg.rrf_k_base == 60
        assert cfg.alpha_min == pytest.approx(0.25)

    def test_no_generic_dict_constructor_on_domain_record(self):
        assert not hasattr(FusionConfig, "from_dict")
